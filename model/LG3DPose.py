...
class Encoder(nn.Module):
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 temporal_connection_len=1, use_tcn=False, graph_only=False, neighbour_num=4, n_frames=243, depth=0):
        super().__init__()
        ...
        # ST Attention branch
        self.att_spatial = Block(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                         qk_scale, use_layer_scale, layer_scale_init_value,
                                         mode='spatial', mixer_type="attention",
                                         use_temporal_similarity=use_temporal_similarity,
                                         neighbour_num=neighbour_num,
                                         n_frames=n_frames)
        self.att_temporal = Block(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                          qk_scale, use_layer_scale, layer_scale_init_value,
        ...
        if graph_only:
            ...
            self.graph_spatial = Block(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads,
                                               qkv_bias,
                                               qk_scale, use_layer_scale, layer_scale_init_value,
                                               ...
                                               neighbour_num=neighbour_num,
                                               n_frames=n_frames)
            self.graph_temporal = Block(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads,
                                                qkv_bias,
                                                qk_scale, use_layer_scale, layer_scale_init_value,
                                                mode='temporal', mixer_type="attention",
                                                use_temporal_similarity=use_temporal_similarity,
    ...
    def forward(self, x):
        ...
        if self.hierarchical:
            ...
            if self.depth in [0, 1, 2, 3]:
                x_graph_spatial = rearrange(x_graph_spatial, 'B (N T) J C  -> (B N) T J C', T=9)
            elif self.depth in [4, 5, 6, 7]:
                x_graph_spatial = rearrange(x_graph_spatial, 'B (N T) J C  -> (B N) T J C', T=27)
            elif self.depth in [8, 9, 10, 11]:
                x_graph_spatial = rearrange(x_graph_spatial, 'B (N T) J C  -> (B N) T J C', T=81)

            x_graph = self.graph_temporal(x_graph_spatial)
            x_graph = rearrange(x_graph, '(B N) T J C  -> B (N T) J C', B=B)
...
class LG3DPose(nn.Module):
    def __init__(self, n_layers, dim_in, dim_feat, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
        ...
        self.fusion = nn.Linear(dim_feat * 4, 4)
        self._init_fusion()

        # self.fusion_transformer = Attention(dim_feat*4, dim_feat, num_heads, qkv_bias, qkv_scale, attn_drop,
        #                            proj_drop=drop, mode='temporal')

    def _init_fusion(self):
        self.fusion.weight.data.fill_(0)
        self.fusion.bias.data.fill_(0.25)
    ...
    def forward(self, x, return_rep=False):
        ...
        i = 0
        fusion_feature = torch.zeros(4, b, t, j, c).cuda()
        ...
        x_fusion = torch.cat((fusion_feature[0], fusion_feature[1], fusion_feature[2], fusion_feature[3]), dim=-1)
        x_fusion = self.fusion(x_fusion)
        x_fusion = x_fusion.softmax(dim=-1)
        # x_fusion = nn.functional.normalize(x_fusion, p=1.0, dim=-1).abs()
        x = fusion_feature[0] * x_fusion[..., 0:1] + fusion_feature[1] * x_fusion[..., 1:2] +\
            fusion_feature[2] * x_fusion[..., 2:3] + fusion_feature[3] * x_fusion[..., 3:4]

        # ################
        # x_mean = x_fusion.view(-1, 4)
        # x_mean = x_mean.mean(dim=0)
        # x_mean = x_mean.view(1, 4)
        # ##################

        ########hard fusion###########
        # x = fusion_feature[0] * 0.25 + fusion_feature[1] * 0.25 + \
        #     fusion_feature[2] * 0.25 + fusion_feature[3] * 0.25

        ##########transformer fusion####################
        # x_fusion = torch.cat((fusion_feature[0], fusion_feature[1], fusion_feature[2], fusion_feature[3]), dim=-1)
        # x = self.fusion_transformer(x_fusion)
