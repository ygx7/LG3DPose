...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/h36m/LG3DPose.yaml", help="Path to the config file.")
    ...
    parser.add_argument('--MAEdir', default=None, type=str, help='directory of pretrained MAE model')
...
def evaluate(args, model, test_loader, datareader, device):
    ...
    with torch.no_grad():
        for x, y in tqdm(test_loader):
            ...
            # layer_weight.append(weight.unsqueeze(0).cpu().numpy())
            # layer_weight.append(weight_flip.cpu().numpy())

    # layer_weight_mean = np.concatenate(layer_weight)
    # layer_weight_mean = np.mean(layer_weight_mean, axis=0)
    # print(layer_weight_mean)
    ...
    # print('Layer weight0:', layer_weight_mean[0])
    # print('Layer weight1:', layer_weight_mean[1])
    # print('Layer weight2:', layer_weight_mean[2])
    # print('Layer weight3:', layer_weight_mean[3])
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        if opts.use_wandb:
            wandb.log({
                ...
                # 'eval/layer_weight0': layer_weight[0],
                # 'eval/layer_weight1': layer_weight[1],
                # 'eval/layer_weight2': layer_weight[2],
                # 'eval/layer_weight3': layer_weight[3],
                **joint_label_errors
            }, step=epoch + 1)
