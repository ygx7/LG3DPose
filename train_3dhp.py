...
from loss.pose3d import loss_mpjpe, n_mpjpe, loss_velocity, loss_limb_var, loss_limb_gt, loss_angle, \
    loss_angle_velocity, compute_PCK, compute_AUC
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/mpi/LG3DPose.yaml", help="Path to the config file.")
...
def evaluate(model, test_loader, n_frames):
    ...
    for data in tqdm(test_loader, 0):
        ...
        # compute PCK and AUC
        epoch_pck.append(compute_PCK(out_target.view(1, -1, 17, 3), inference_out.view(1, -1, 17, 3)))
        epoch_auc.append(compute_AUC(out_target.view(1, -1, 17, 3), inference_out.view(1, -1, 17, 3)))
    ...
    pck = np.mean(epoch_pck, axis=0)
    auc = np.mean(epoch_auc, axis=0)
